    ...
    for f in range(num_frames):
        ...
        for command in commands:
            ...
            if c == 'box':
                ...
                newl = symbols[command['light']][1]
                light.append([newl['location'], newl['color']])
                ...
                if command['cs0']:
                    matrix_mult(symbols[command['cs0']][1], [tmp[0]])
                else:
                    matrix_mult( stack[-1], [tmp[0]])
                if command['cs1']:
                    matrix_mult(symbols[command['cs1']][1], [tmp[1]])
                else:
                    matrix_mult( stack[-1], [tmp[1]])
